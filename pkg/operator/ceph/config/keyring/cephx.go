...
func ShouldRotateCephxKeys(cfg v1.CephxConfig, runningCephVersion, desiredCephVersion version.CephVersion, status v1.CephxStatus, ignoreKeyType bool) (bool, error) {
	...
	if !typeUnsetOrLegacy && !cephclient.Aes256kKeysSupported(runningCephVersion) {
		...
		return false, fmt.Errorf(
			"ceph version %#v does not support key type %q; change the requested keyType or updgrade Ceph",
...
func UpdatedCephxStatus(didRotate bool, cfg v1.CephxConfig, runningCephVersion version.CephVersion, status v1.CephxStatus, keyType v1.CephxKeyType) v1.CephxStatus {
	...
	// TODO(key): when `ceph auth dump-keys` is available in a released ceph version, use it to get
	// the definitive key type for the status, and send that from calling functions.
